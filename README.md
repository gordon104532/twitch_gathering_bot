...
6. 查看排行榜 數字可自訂列出幾筆 (http://127.0.0.1:8787/rank?target=100000)
...
### 重大問題
- [ ] (已知)活動開關只會停止計算小奇點，訂閱斗內仍會運作
- [ ] (已知)控制頁開關說明文字會影響歐富寶開關
- [ ] (未解)活動開關會自己關掉
- [ ] (未解)紀錄消失、總分回朔 => 舊蓋新
之後要調整 紀錄(gatTotalPoint) 時，文件開啟後若有分數異動，先不儲存關閉再打開調整並儲存、關閉
之後要調整 控制頁 時，控制頁開著後若有分數異動，先按一下最下方的"還原"按鈕，來取得最新的設定/總分，之後再做"任何"欄位的調整與儲存